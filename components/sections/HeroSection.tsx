...
export default function HeroSection({
  ...
  const onWheelRef = useRef<((event: WheelEvent) => void) | null>(null);
  ...
  useEffect(() => {
    ...
    const initTimer = window.setTimeout(() => {
      ...
      onWheelRef.current = (event: WheelEvent) => {
        if (event.ctrlKey || event.metaKey) {
          ...
          const cursorLngLat = map.unproject([
          ...
          if (zoomEndTimer.current) {
            window.clearTimeout(zoomEndTimer.current);
          }
      ...
      map.on('load', () => {
        ...
        mappedProperties.forEach((property, index) => {
          const timeoutId = window.setTimeout(() => {
            ...
            const element =
              createPinElement(formattedPrice);
    ...
    return () => {
      ...
      markerTimers.current.forEach((timer) =>
...
function createPinElement(
  ...
  element.className =
    'group relative cursor-pointer origin-bottom';
  ...
  element.innerHTML = `
    <div class="relative pb-[7px]">
      <div class="flex items-center gap-[5px] whitespace-nowrap rounded-full border border-stone-900/10 bg-white px-3 py-1.5 shadow-[0_8px_24px_rgba(13,27,42,0.14)] transition duration-200 group-hover:-translate-y-0.5 group-hover:bg-[#fdf3dc]">
        ...
        <span class="text-[12px] font-bold tracking-[-0.01em] text-stone-900">
          ${escapeHtml(price)}
        </span>
      </div>
